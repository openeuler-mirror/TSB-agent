			  ...
			  int result)
{
	...
	if (point) {
		...
		memcpy(sec_d->sub_name, ACTION_NAME, strlen(ACTION_NAME));
	}
...
static int backup_syscall(void)
{
	...
	sys_m = kzalloc(sizeof(struct syscall_measure) + data_len, GFP_KERNEL);
	if (!sys_m) {
		ret = -ENOMEM;
		goto out;
	}
...
int syscall_table_check(void *data)
{
	...
	if (ret == 0) {
		...
		send_audit_log(point, ACTION_NAME, RESULT_SUCCESS);
	...
	for (i = 0; i < NR_syscalls; i++) {
		if (sys_m->backup_syscall_table[i] !=
		    (unsigned long *)sys_call[i]) {
