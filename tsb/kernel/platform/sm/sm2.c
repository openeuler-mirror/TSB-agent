...
static inline void sm2_shared_key(ecc_point *point, u8 *ZA, u8 *ZB, u32 keyLen, u8 *key);
static int ECC_Key_ex_hash2(u8 temp, u8* y,u8 *hash, u8* SA);
static inline int sm2_point_mult(ecc_point *G, u8 *k, ecc_point *P);
//#include "tcm_debug.h"
...
static void ecc_b2e(u8 *e, const u8 *b)
{
	unsigned int i;
...
static void sm2_w(u8 *result, u8 *x)
{
	memcpy(result, x, 16);
	result[15] |= 0x80;
...
static void sm3_z(const u8 *id, u32 idlen, ecc_point *pub, u8 *hash)
{
	u8 pubx[ECC_NUMWORD];
	u8 puby[ECC_NUMWORD];
	u8 a[ECC_NUMWORD];
...
static int sm2_make_prikey(u8 *prikey)
{
	u8 pri[ECC_NUMWORD];
	int i = 10;

	do {
		get_random_bytes(pri, ECC_NUMWORD);
...
static int sm2_make_pubkey(u8 *prikey, ecc_point *pubkey)
{
	ecc_point pub[1];
	u8 pri[ECC_NUMWORD];
...
static int sm2_sign_impl(u8 *r, u8 *s, const u8 *prikey, const u8 *hash)
{
	u8 Rng[ECC_NUMWORD] = {
	...
	get_random_bytes(random, ECC_NUMWORD);
...
static int sm2_verify_impl(ecc_point *pubkey, const u8 *hash,  const u8 *r, const u8 *s)
{
	ecc_point result;
	u8 t[ECC_NUMWORD];
	u8 rr[ECC_NUMWORD];
...
//int sm2_encrypt(ecc_point *pubKey, u8 *M, u32 Mlen, u8 *C, u32 *Clen)
	...
	u8 *C3 = (C+1) + ECC_NUMWORD*2;
	u8 *C2 = (C+1) + ECC_NUMWORD*2 + ECC_NUMWORD;
	ecc_point S;
...
#ifndef STANDARD_PART_5
	get_random_bytes(Rng, ECC_NUMWORD);
#endif
	ecc_b2e(Rng, Rng);
...
//int sm2_decrypt(u8 *prikey, u8 *C, u32 Clen, u8 *M, u32 *Mlen)
{
	...
	u8 hash[ECC_NUMWORD] ={0};
	u8 pri[ECC_NUMWORD] ={0};
...
//	ENTER();
	...
	{
		//pr_dev("ecc_valid_public_key error\n");
		return -1;
	}
	...
	if (ecc_valid_public_key(&S) != 0)
	{
		//pr_dev("ecc_valid_public_key(&S) error\n");
		return -1;
	}
...
static int sm2_shared_point(const u8* selfPriKey,  const u8* selfTempPriKey, ecc_point* selfTempPubKey,
...
static int ECC_Key_ex_hash1(u8* x, ecc_point *RA, ecc_point* RB,const  u8 ZA[],const u8 ZB[], u8 *hash)
{
	sm3_ctx_common md[1];
					...
					unsigned char pubkey_XY[64])
{
	...
	u8 e[32] = {0};
	u8 Za[32] = {0};
	//u8 id[18] = "ALICE123@YAHOO.COM";
	//ecc_point pub;
	sm3_ctx_common ctx;
	sm3_z(id, lenUID, (ecc_point *)pubkey_XY, Za);
	mlen = ilen + 32;
	m = kmalloc(mlen,GFP_KERNEL);
	if(m)
	{
		memcpy(m, Za, 32);
		...
		DEBUG_MSG(HTTC_TSB_INFO,"No memory for SM2Sign\n");
		return -ENOMEM;
	}
}
...
//	ecc_point Rb;
	...
	if(uid_len_a !=32 ||  uid_len_b !=32  || prikey_len_a !=32  || rndKeyLen_A !=32  ){
		//pr_dev("sm2_dh_key param length error\n");
		return -1;
	}
...
//	memcpy(Rb.y, R_b->y, SM2_BIGNUM_BUFSIZE);
	...
	if(role == ROLE_REQUESTOR){
		...
		ECC_Key_ex_hash1(UV.x,  (ecc_point *)pubkey_temp_a, (ecc_point *)pubkey_temp_b, (u8 *)uid_a, (u8 *)uid_b, hash1);
