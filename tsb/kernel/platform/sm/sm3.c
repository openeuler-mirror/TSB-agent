...
static void SM3_CF(struct  sm3_context *ctx, const unsigned char *byte_64_block )
{
	uint32_t j,W[68];
	...
	for(j=16;j<68;j++)
	{
		...
		printf("%u::(*)=%8x :temp=%8x :P1((*))=%8x :P1(*)=%8x :p1(temp)=%8x\n",
...
#define GG(X,Y,Z) (X^Y^Z)
	...
	for(j=0;j<16;j++)
	{
		...
		E = P0(TT2);
#ifdef DEBUG_PRINT
		printf("%u: A:%08x,B:%08x,C:%08x,D:%08x,E:%08x,F:%08x,G:%08x,H:%08x\n",j,A,B,C,D,E,F,G,H);
#endif
	}
...
int ht_sm3_init(void *ctx0){
	struct   sm3_context *ctx = (struct  sm3_context *)ctx0;
	if ( ctx == NULL )
	{
		return -1;
	}
...
EXPORT_SYMBOL(ht_sm3_init);
int ht_sm3_update(void *ctx0, const unsigned char *chunk_data, int chunk_length)
...
EXPORT_SYMBOL(ht_sm3_update);
int ht_sm3_finish(void *ctx0, unsigned char *output)
//int sm3_finish (sm3_context *ctx, unsigned char output[SM3_DIGEST_SIZE])
{
  ...
  ret = ht_sm3_update( ctx, SM3_padding, padn );
  if (ret != 0)
  	return ret;
  
  ret = ht_sm3_update( ctx, msglen, 8 );
  if (ret != 0)
  	return ret;
...
EXPORT_SYMBOL(ht_sm3_finish);
///int sm3_finup(void *ctx0,const unsigned char *input, int ilen, unsigned char *output);
//int sm3_finup(void *ctx0,const unsigned char *input, int ilen, unsigned char *output)
////void sm3 (const unsigned char *input, int ilen,
////           unsigned char output[SM3_DIGEST_SIZE])
//{
//	//sm3_context *ctx = (sm3_context *)ctx0;
//
//    sm3_context ctx;
...
//    return 0;
//}
//int ht_sm3_check(int size){
//	tpcm_debug("sm3 size required %d,configured %d\n",sizeof(sm3_context),size);
//	return (int)sizeof(sm3_context) > CONFIG_SM3_CONTEXT_MAX_SIZE ? -1 :0;
//}
