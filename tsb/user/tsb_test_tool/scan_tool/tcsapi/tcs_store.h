...
enum{
	 ...
	 PCR_TYPE_INIT_ROOT,   //pcr7
	 PCR_TYPE_BOOT_CONFIG, //pcr8
};
...
int tcs_nv_define_named_space(
		...
		unsigned char *ownerpasswd,
		unsigned char *usepasswd,uint32_t *index);
...
int tcs_nv_define_named_space_on_policy(
		const char *name,	int size,
		unsigned char *ownerpasswd,
		struct auth_policy *policy,uint32_t *index);
...
int tcs_is_nv_defined(uint32_t index);
