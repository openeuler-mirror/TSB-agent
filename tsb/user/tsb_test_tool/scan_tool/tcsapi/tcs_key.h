...
int tcs_sign(
		...
		unsigned char *obuffer,	int *oleng_inout);
 ...
 * 	创建迁移密钥	
 */
int tcs_create_migratable_key(unsigned char *key_path,uint32_t type);
 ...
 *	准备认证数据
 */
int tcs_get_migrate_auth(const char *keypath, unsigned char *ownerpass, unsigned char **auth,	int *authlength);

/*
 *	迁出密钥树
 */
int tcs_emigrate_keytree(const char *keypath, unsigned char *passwd,unsigned char *auth,	int authlength,unsigned char **pbuffer, int *obuf_len);

/*
 * 	迁入密钥树
 */
int tcs_immigrate_keytree(const char *keypath, const char *migrate_keypath, 
													unsigned char *pbuffer,int buf_len);
