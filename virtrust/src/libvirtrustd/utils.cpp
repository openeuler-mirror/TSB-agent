...
bool IsAbsolutePath(const std::string &filePath) {
  ...
  if (strstr(filePath.c_str(), "/../") != nullptr ||
      strstr(filePath.c_str(), "/./") != nullptr) {
    return false;
  }
  return true;
}
...
std::optional<rapidjson::Document> ReadJsonFile(const std::string &path) {
  ...
  if (!CheckConfigFile(realPath.value())) {
    VIRTRUST_LOG_ERROR("Read config error. File access check failed: {}",
                       realPath.value());
    return std::nullopt;
  }
  ...
  if (!file.is_open()) {
    VIRTRUST_LOG_ERROR("Read config error. Cannot open file: {}",
                       realPath.value());
    return std::nullopt;
  }
  ...
  if (doc.HasParseError()) {
    VIRTRUST_LOG_ERROR("Read config error. Cannto parse file into json: {}",
                       realPath.value());
    return std::nullopt;
  }
...
template <typename T>
std::optional<T> FindJsonKey(const rapidjson::Document &jsonDoc,
                             const std::string &key, T defaultValue) {
  if (!jsonDoc.HasMember(key.data())) {
    VIRTRUST_LOG_WARN("ParseJsonDocToConfig: Faild to find config key:{}, Set "
                      "it to default value.")
    return defaultValue;
  }
  ...
  if constexpr (std::is_same_v<T, std::string>) {
    return value.IsString() ? std::optional<T>(std::string(value.GetString()))
                            : std::nullopt;
