...
const std::vector<std::string> SM3_TEST_DATA =
  ...
  for (unsigned char byte : bytes) {
    oss << std::setw(2)
        << static_cast<int>(byte); // Format each bute as 2-digit hex
  }
...
TEST(Sm3Test, ResetFunctionality) {
  ...
  EXPECT_NE(memcmp(intermediateHash.data(), finalHash.data(),
                   intermediateHash.size()),
            0);
}
...
TEST(Sm3Test, HashConcatenation) {
  ...
  // Hashes should be different
  EXPECT_NE(memcmp(hash1Result.data(), hash2Result.data(), hash1Result.size()),
            0);
  EXPECT_NE(memcmp(hash1Result.data(), hash3Result.data(), hash1Result.size()),
            0);
  EXPECT_NE(memcmp(hash2Result.data(), hash3Result.data(), hash2Result.size()),
            0);
}
} // namespace virtrust::test