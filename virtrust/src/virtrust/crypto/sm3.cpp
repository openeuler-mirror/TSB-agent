...
Sm3::Sm3()
    : md_(SmartEVP_MD(
          Openssl::GetInstance().EVP_MD_fetch(nullptr, "sm3", nullptr))),
...
Sm3Rc Sm3::Update(std::string_view data) {
  if (data.size() > UPDATE_SIZE_LIMIT) {
    VIRTRUST_LOG_ERROR(
        "Sm3::Update() Failed, input data size:{}, exceeds limit:{}",
        data.size(), UPDATE_SIZE_LIMIT);
    return Sm3Rc::ERROR;
  }
  ...
  auto ret = Openssl::GetInstance().EVP_DigestUpdate(context_.Get(),
                                                     data.data(), data.size());
  if (ret != OPENSSL_OK) {
    VIRTRUST_LOG_ERROR("Sm3::Update() Failed, EVP_DigestUpdate  returns: {}",
                       ret);
    return Sm3Rc::ERROR;
  }
...
std::vector<uint8_t> Sm3::CumulativeHash() const {
  if (context_.Get() == nullptr || md_.Get() == nullptr) {
    VIRTRUST_LOG_ERROR(
        "Sm3::CumulativeHash() Failed, context_ or md_ is nullptr");
    return {};
  }
  ...
  if (ctx_snapshot.Get() == nullptr) {
    VIRTRUST_LOG_ERROR(
        "Sm3::CumulativeHash() Failed, EVP_MD_CTX_new returns nullptr");
    return {};
  }

  auto ret = Openssl::GetInstance().EVP_MD_CTX_copy_ex(ctx_snapshot.Get(),
                                                       context_.Get());
  if (ret != OPENSSL_OK) {
    VIRTRUST_LOG_ERROR(
        "Sm3::CumulativeHash() Failed, EVP_MD_CTX_copy_ex returns:{}", ret);
    return {};
  }

  ret = Openssl::GetInstance().EVP_DigestFinal_ex(ctx_snapshot.Get(),
                                                  out.data(), &out_len);
  if (ret != OPENSSL_OK) {
    VIRTRUST_LOG_ERROR(
        "Sm3::CumulativeHash() Failed, EVP_DigestFinal_ex returns:{}", ret);
    return {};
  }
...
std::array<uint8_t, Sm3::DigestSize()> DoSm3(std::string_view data) {
  ...
  if (buf.size() < Sm3::DigestSize()) {
    VIRTRUST_LOG_ERROR("DoSm3() Failed, CumulativeHash output len incorrent:{}",
                       buf.size());
    return {};
  }
