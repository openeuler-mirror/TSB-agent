...
namespace {
constexpr std::string_view DEFAULT_SHIM_EFI_REGEX_PATH =
    "/boot/efi/EFI/{}/shimaa64.efi";
constexpr std::string_view DEFAULT_GRUB_EFI_REGEX_PATH =
    "/boot/efi/EFI/{}/grubaa64.efi";
constexpr std::string_view DEFAULT_GRUB_CFG_REGEX_PATH =
...
VerifyConfig::VerifyConfig(const std::string &guestName,
                           const std::string &diskPath,
                           const std::string &loaderPath,
                           const LinuxDistro distro)
    : guestName_(guestName), diskPath_(diskPath), loaderPath_(loaderPath),
      linuxDistro_(distro) {}

std::string VerifyConfig::GetGuestName() { return guestName_; }

std::string VerifyConfig::GetDiskPath() { return diskPath_; }

std::string VerifyConfig::GetLoaderPath() { return loaderPath_; }

std::string VerifyConfig::GetShimPath() {
  if (shimPath_.empty()) {
    shimPath_ = fmt::format(DEFAULT_SHIM_EFI_REGEX_PATH,
                            LinuxDistroToString(linuxDistro_));
  }
  return shimPath_;
}
...
std::string VerifyConfig::GetGrubCfgPath() {
  if (grubCfgPath_.empty()) {
    grubCfgPath_ = fmt::format(DEFAULT_GRUB_CFG_REGEX_PATH,
                               LinuxDistroToString(linuxDistro_));
  }
  return grubCfgPath_;
}

std::string VerifyConfig::GetInitrdPath() { return initrdPath_; }

std::string VerifyConfig::GetLinuzPath() { return linuzPath_; }
  ...
  while (std::getline(iss, line)) {
    ...
    if (StrStartsWith(line, DEFAULT_INITRD_PREFIX)) {
      ...
      if (initrdPath_.empty()) {
        VIRTRUST_LOG_ERROR(
            "|ParseGrubCfgContent|END|||Get initrd path from grub.cfg failed.");
        return;
      }
      findInitrd = true;
      ...
      if (linuzPath_.empty()) {
        VIRTRUST_LOG_ERROR(
            "|ParseGrubCfgContent|END|||Get linuz path from grub.cfg failed.");
        return;
      }
      findLinuz = true;
    }
...
std::string VerifyConfig::ParseGrubCfgLine(const std::string &line) {
  ...
  // add prefix
  return fmt::format("{}/{}", DEFAULT_BOOT_PATH,
                     StrTrim(filename, PATH_SEPARATOR));
}
...
ForeignMounterRc ForeignMounter::TryInit() {
  ...
  if (handle_ == nullptr) {
    VIRTRUST_LOG_ERROR(
        "|TryInit|END|returnF||Failed to create the guestfs handle.");
    return ForeignMounterRc::ERROR;
  }
  return ForeignMounterRc::OK;
}

ForeignMounterRc ForeignMounter::Mount(std::string_view imgPath,
                                       size_t osIndex) {
  ...
  struct guestfs_add_drive_opts_argv optargs = {
      .bitmask = GUESTFS_ADD_DRIVE_OPTS_FORMAT_BITMASK |
  ...
  res =
      libguestfs_.guestfs_add_drive_opts_argv(handle_, imgPath.data(), optargs);
  if (res == -1) {
  ...
  if (roots == nullptr) {
    VIRTRUST_LOG_ERROR(
        "|Mount|END|returnF||No operating system found in image.");
    return ForeignMounterRc::ERROR;
  }
  ...
  if (osIndex >= osCnt) {
    VIRTRUST_LOG_ERROR(
        "|Mount|END|returnF||There are {} os in image, index out of range: {}.",
        osCnt, osIndex);
    return ForeignMounterRc::ERROR;
  }
...
ForeignMounterRc ForeignMounter::MountFilesystems(const std::string &root) {
  ...
  char **mountpoints =
      libguestfs_.guestfs_inspect_get_mountpoints(handle_, root.c_str());
  if (mountpoints == nullptr) {
    VIRTRUST_LOG_ERROR(
        "|MountFilesystems|END|returnF||Get mountpoints failed.");
    return ForeignMounterRc::ERROR;
  }
  std::vector<MounterInfo> mps;
  constexpr int stepLen = 2;
  for (int i = 0; mountpoints[i] != nullptr && mountpoints[i + 1] != nullptr;
       i += stepLen) {
    mps.push_back(MounterInfo{mountpoints[i], mountpoints[i + 1],
                              StrCountChar(mountpoints[i], PATH_SEPARATOR)});
  }

  // sort by directory level in ascending
  std::sort(mps.begin(), mps.end(), [](auto &a, auto &b) {
    return a.dirLevel != b.dirLevel ? a.dirLevel < b.dirLevel
                                    : a.mpt.size() < b.mpt.size();
  });
  ...
  for (auto &mp : mps) {
    ...
    int ret = libguestfs_.guestfs_mount_ro(handle_, device.c_str(),
                                           mountpoint.c_str());
    if (ret == -1) {
      VIRTRUST_LOG_ERROR(
          "|MountFilesystems|END|returnF||Mount {} to {} failed.", device,
          mountpoint);
      return ForeignMounterRc::ERROR;
    }
  }
...
ForeignMounterRc ForeignMounter::ReadFile(std::string_view filePath,
  ...
  if (exists != 1) {
    VIRTRUST_LOG_ERROR(
        "|ReadFile|END||file path is: {}|File not exist in image.", filePath);
  ...
  if (isFile != 1) {
    VIRTRUST_LOG_ERROR("|ReadFile|END|||file path is: {}|It is not a file.",
                       filePath);
    return ForeignMounterRc::ERROR;
  }
  ...
  size_t size;
  char *content =
      libguestfs_.guestfs_read_file(handle_, filePath.data(), &size);
  if (content == nullptr) {
    VIRTRUST_LOG_ERROR(
        "|ReadFile|END|||file path is: {}|Read this file failed.", filePath);
    return ForeignMounterRc::ERROR;
  }
...
ForeignMounterRc ForeignMounter::DoSm3OnVmFile(std::string_view filePath,
  ...
  if (rc != ForeignMounterRc::OK) {
    VIRTRUST_LOG_ERROR("|DoSm3OnVmFile|END|returnF||Read file failed: {}.",
                       filePath);
    return ForeignMounterRc::ERROR;
  }
...
ForeignMounterRc ForeignMounter::EnableStrErrTrace() {
  return libguestfs_.guestfs_set_trace(handle_, 1) == 0
             ? ForeignMounterRc::OK
             : ForeignMounterRc::ERROR;
}

ForeignMounterRc ForeignMounter::DisableStrErrTrace() {
  return libguestfs_.guestfs_set_trace(handle_, 0) == 0
             ? ForeignMounterRc::OK
             : ForeignMounterRc::ERROR;
}
