...
TEST_F(AsyncTimerTest, StartWithMicrosecondTimeout)
{
    ...
    timer.Start(std::chrono::microseconds(5000), [this]() {
        IncrementCallback();
    });
...
TEST_F(AsyncTimerTest, StartWithZeroTimeout)
{
    ...
    timer.Start(std::chrono::microseconds(0), [this]() {
        IncrementCallback();
    });
...
TEST_F(AsyncTimerTest, CancelBeforeTimeout)
{
    ...
    timer.Start(std::chrono::milliseconds(200), [this]() {
        IncrementCallback();
    });
...
TEST_F(AsyncTimerTest, LongTimeout)
{
    ...
    timer.Start(std::chrono::milliseconds(500), [this]() {
        IncrementCallback();
    });
...
TEST_F(AsyncTimerTest, MultipleTimersConcurrent)
{
    ...
    timer1.Start(std::chrono::milliseconds(50), [this]() {
        callback_value_ += 1;
    });
    
    timer2.Start(std::chrono::milliseconds(100), [this]() {
        callback_value_ += 10;
    });
    
    timer3.Start(std::chrono::milliseconds(150), [this]() {
        callback_value_ += 100;
    });
...
TEST_F(AsyncTimerTest, TimerScopeAndDestruction)
{
    ...
    
    {
        ...
        timer.Start(std::chrono::milliseconds(50), [&destruction_count]() {
            destruction_count++;
        });
...
TEST_F(AsyncTimerTest, PrecisionTest)
{
    ...
    timer.Start(std::chrono::milliseconds(10), [&callback_called]() {
