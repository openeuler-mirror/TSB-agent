...
#define FILE_IO_THROW(msg_prefix)                                              \
  VIRTRUST_ENFORCE(                                                            \
      false, fmt::format("|{}|END|||error on file {}, failure {}", msg_prefix, \
                         fileName_, e.what(), std::strerror(errno), errno))
...
size_t FileInputStream::GetLength() const { return fileLen_; }

const std::string &FileInputStream::GetName() const { return fileName_; }
...
FileOutputStream::FileOutputStream(std::string fileName, bool trunc,
                                   bool exitFailInDestructor)
    : fileName_(std::move(fileName)),
      exitFailInDestructor_(exitFailInDestructor) {
  ...
  if (!fp.parent_path().empty() && !std::filesystem::exists(fp.parent_path())) {
    VIRTRUST_ENFORCE(std::filesystem::create_directories(fp.parent_path()),
...
const std::string &FileOutputStream::GetName() const { return fileName_; }
