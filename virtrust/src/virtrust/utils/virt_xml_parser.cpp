...
void VirtXmlParser::SearchCurrLevel(std::vector<xmlNodePtr> &currLevel,
  ...
  for (xmlNodePtr parent : currLevel) {
    for (xmlNodePtr node = parent->children; node != nullptr;
         node = node->next) {
      bool pathMatch =
          node->type == XML_ELEMENT_NODE && nodeName == MakeString(node->name);
      if (pathMatch) {
...
VirtXmlParser::FindNodesByPath(std::string_view absPath) {
  ...
  if (absPath[0] != '/') {
    VIRTRUST_LOG_ERROR(
        "|FindNodesByPath|END||file path: {}|It's not a absolutely path.",
        absPath);
    return ret;
  }

  std::vector<std::string> parts = StrSplit(
      StrTrim(std::string(absPath), PATH_SEPARATOR[0]), PATH_SEPARATOR);
...
bool VirtXmlParser::LoadFile(std::string_view filename) {
  ...
  if (!std::filesystem::exists(filename) ||
    ...
    VIRTRUST_LOG_ERROR("|LoadFile|END|returnF||The file does not exist: {}.",
                       filename);
    return false;
  }
...
std::string VirtXmlParser::GetDiskPath() {
  ...
  if (nodes.empty()) {
    VIRTRUST_LOG_ERROR(
        "|GetDiskPath|END||node path: {}|Get disk path from xml failed.",
        VIRT_XML_DISK_PATH);
    return "";
  }
  ...
  if (nodes.size() != 1) {
    VIRTRUST_LOG_ERROR(
        "|GetDiskPath|END|||The node of disk path in xml is not only.");
    return "";
  }
...
std::string VirtXmlParser::GetLoaderPath() {
  ...
  if (nodes.empty()) {
    VIRTRUST_LOG_ERROR(
        "|GetLoaderPath|END|||Get loader path from xml failed: {}.",
  ...
  if (nodes.size() != 1) {
    VIRTRUST_LOG_ERROR(
        "|GetLoaderPath|END|||The node of loader path in xml is not only.");
    return "";
  }
...
std::string VirtXmlParser::GetVmName() {
  ...
  if (nodes.size() != 1) {
    VIRTRUST_LOG_ERROR(
        "|GetVmName|END|||The node of VM name in xml is not only.");
    return "";
  }
