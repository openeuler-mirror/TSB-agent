...
TEST(Exception, CompareWithMessages) {
  try {
    ...
    EXPECT_NE(std::string(e.what()).find("Excepted 1 to equal 2"),
              std::string::npos);
  }
  ...
  try {
    ...
    EXPECT_NE(std::string(e.what()).find("1 vs 2"), std::string::npos);
    EXPECT_NE(std::string(e.what()).find("1 should be greater than 2"),
              std::string::npos);
  }
}
...
TEST(Exception, EnforceThat) {
  ...
  ASSERT_NO_THROW(VIRTRUST_ENFORCE_THAT(enforce_detail::Equals(1, 1),
