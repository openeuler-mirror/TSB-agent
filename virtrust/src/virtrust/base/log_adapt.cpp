...
VirtrustRc LogAdapt::CreateInstance(int &logType, int logLevel,
  ...
  if (path != nullptr) {
    ...
    if (!Exist(folderPath, F_OK | R_OK | W_OK)) {
      std::cerr << "Access denied or logpath: " << folderPath
  ...
  VirtrustRc ret = SetTmpLogger(logType, realPath, rotationFileSize,
...
VirtrustRc LogAdapt::Log(int level, const char *prefix,
  ...
  this->spdLogger->log(static_cast<spdlog::level::level_enum>(level), "{}]{}",
                       prefix, message);
  return VirtrustRc::OK;
}
