...
public:
  ...
  DlFun(std::string_view name, FunTy funptr)
      : funName_(name), funptr_(funptr ? std::function<R(Args...)>(funptr)
                                       : std::function<R(Args...)>(nullptr)) {}
...
public:
  ...
  DllibRc CheckOk() const { return ok_ ? DllibRc::OK : DllibRc::ERROR; }
