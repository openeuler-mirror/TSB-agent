...
namespace virtrust {
    ...
    TEST(DynamicLibraryTest, LibvirtInitialization)
    {
        ...
        // Note: we don't assert CheckOK() as libvirt might not be available in test environment
    ...
    TEST(DynamicLibraryTest, LibguestfsInitialization)
    {
        ...
        // Note: we don't assert CheckOK() as libguestfs might not be available in test environment
        // But we can verify it doesn't crash
    ...
    TEST(DynamicLibraryTest, Libxml2Initialization)
    {
        ...
        // Note: we don't assert CheckOK() as libxml2 might not be available in test environment
        // But we can verify it doesn't crash
