...
public:
  ...
  VRootData GetData() const { return data_; }

  VRootData &GetDataRef() { return data_; }

  Description GetDescription() {
    Description out;
    out.state = static_cast<int>(data_.status);
    memcpy_s(out.name, VROOT_VM_NAME_MAX_LEN, data_.name.data(),
             data_.name.size());
    memcpy_s(out.uuid, VROOT_VM_UUID_MAX_LEN, data_.uuid.data(),
             data_.uuid.size());
    return out;
  }
