...
int GetVRoots(int *vtpcmNums, struct Description **vtpcmInfo) {
  ...
  *vtpcmInfo =
      static_cast<Description *>(malloc(*vtpcmNums * sizeof(Description)));
...
int CreateVRoot(struct Description *vtpcmInfo) {
  auto &tsbAgent = TsbAgentImpl::GetInstance();
  return ParseTsbAgentRc(
      tsbAgent.CreateVRoot(vtpcmInfo->uuid, vtpcmInfo->name));
}
...
int UpdateMeasure(char *uuid, struct MeasureInfo *bios,
                  struct MeasureInfo *shim, struct MeasureInfo *grub,
                  struct MeasureInfo *grubCfg, struct MeasureInfo *kernel,
                  struct MeasureInfo *initrd) {
  auto &tsbAgent = TsbAgentImpl::GetInstance();
  return ParseTsbAgentRc(tsbAgent.UpdateMeasure(uuid, *bios, *shim, *grub,
                                                *grubCfg, *kernel, *initrd));
}

int CheckMeasure(char *uuid, struct MeasureInfo *bios, struct MeasureInfo *shim,
                 struct MeasureInfo *grub, struct MeasureInfo *grubCfg,
                 struct MeasureInfo *kernel, struct MeasureInfo *initrd) {
  auto &tsbAgent = TsbAgentImpl::GetInstance();
  return ParseTsbAgentRc(tsbAgent.CheckMeasure(uuid, *bios, *shim, *grub,
                                               *grubCfg, *kernel, *initrd));
}

// 迁移接口
int GetReport(char *pUuid, char *vUuid, struct trust_report_new *hostreport,
...
int MigrationNotify(char *pUuid, char *vUuid, int status) { return 0; }
