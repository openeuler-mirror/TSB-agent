...
void SaveVRootsToFile(
    const std::string &file,
    const std::unordered_map<std::string, std::unique_ptr<VRoot>> &map) {
  std::ofstream f(file);
  if (f.is_open()) {
    // encode line format
    for (const auto &item : map) {
      f << ToStr(item.second->GetData().uuid) << TSB_AGENT_FILE_SEP; // uuid
      f << static_cast<uint32_t>(item.second->GetData().status)
        << TSB_AGENT_FILE_SEP; // status        }
      f << ToStr(item.second->GetData().version)
        << TSB_AGENT_FILE_SEP;                                       // version
      ...
      f << ToHexStr(item.second->GetData().grubCfg)
        << TSB_AGENT_FILE_SEP; // grubCfg
      f << ToHexStr(item.second->GetData().kernel)
        << TSB_AGENT_FILE_SEP; // kernel
      f << ToHexStr(item.second->GetData().initrd)
        << TSB_AGENT_FILE_SEP; // initrd
      f << std::endl;
    }
    f.close();
  }
...
std::unordered_map<std::string, std::unique_ptr<VRoot>>
LoadVRootsFromFile(const std::string &file) {
  // placeholder
  ...
  if (f.is_open()) {
    while (getline(f, line)) {
      ...
      data.status =
          static_cast<VRootStatus>(std::atoi(lineVec[1].c_str()));   // status
...
TsbAgentRc TsbAgentImpl::CreateVRoot(const std::string &uuid,
  ...
  if (std::find_if(vRootMap_.begin(), vRootMap_.end(),
                   [name](const auto &root) {
  ...
  VRootData desc;
  if (memcpy_s(desc.uuid.data(), desc.uuid.size(), uuid.data(), uuid.size()) !=
      EOK) {
  ...
  if (memcpy_s(desc.name.data(), desc.name.size(), name.data(), name.size()) !=
      EOK) {
  ...
  if (memcpy_s(desc.version.data(), desc.version.size(), DEFAULT_VERSION.data(),
               DEFAULT_VERSION.size()) != EOK) {
...
TsbAgentRc TsbAgentImpl::UpdateMeasure(const std::string &uuid,
                                       MeasureInfo bios, MeasureInfo shim,
                                       MeasureInfo grub, MeasureInfo grubCfg,
                                       MeasureInfo kernel, MeasureInfo initrd) {
  vRootMap_ = LoadVRootsFromFile(storageFilePath_.c_str());
  if (vRootMap_.find(uuid) == vRootMap_.end()) {
    SPDLOG_ERROR("[MOCK-TSB-AGENT] uuid:{} not found", uuid);
    return TsbAgentRc::ERROR;
...
TsbAgentRc TsbAgentImpl::CheckMeasure(const std::string &uuid, MeasureInfo bios,
                                      MeasureInfo shim, MeasureInfo grub,
                                      MeasureInfo grubCfg, MeasureInfo kernel,
                                      MeasureInfo initrd) {
  vRootMap_ = LoadVRootsFromFile(storageFilePath_.c_str());
  if (vRootMap_.find(uuid) == vRootMap_.end()) {
    SPDLOG_ERROR("[MOCK-TSB-AGENT] uuid:{} not found", uuid);
