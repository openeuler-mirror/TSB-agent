...
int tcf_create_sign_key(
	...
	ret = tcs_create_sign_key(key_path,type,passwd);
	if(ret) return ret;
	
	httc_write_source_notices (source, POLICY_TYPE_KEYTREE);	
	return ret;
}
...
int tcf_create_sign_key_on_policy(
	...
	ret =  tcs_create_sign_key_on_policy(key_path,type,policy);	
	if(ret) return ret;
...
int tcf_sign(
	...
	ret = tcs_sign(key_path,passwd,ibuffer,ilength,obuffer,olen_inout);
...
int tcf_create_encrypt_key(
	...
	ret =  tcs_create_encrypt_key(key_path,type,passwd);
...
int tcf_create_inner_encrypt_key(
	...
	ret =  tcs_create_inner_encrypt_key(key_path,type,passwd);
...
int tcf_create_encrypt_key_on_policy(
	...
	ret =  tcs_create_encrypt_key_on_policy(key_path,type,policy);
...
int tcf_create_inner_encrypt_key_on_policy(
	...
	ret =  tcs_create_inner_encrypt_key_on_policy(key_path,type,policy);
...
int tcf_encrypt(
	...
	ret = tcs_encrypt(key_path,passwd,mode,ibuffer,ilength,obuffer,olen_inout);
...
int tcf_decrypt(
	...
	ret = tcs_decrypt(key_path,passwd,mode,ibuffer,ilength,obuffer,olen_inout);
...
int tcf_set_encrypt_key(
	...
	ret =  tcs_set_encrypt_key(key_path,passwd,length,key);
...
int tcf_get_encrypt_key(
	...
	ret = tcs_get_encrypt_key(key_path,passwd,length,key);
...
int tcf_create_seal_key(
	...
	ret =  tcs_create_seal_key(key_path,type,passwd);
...
int tcf_create_seal_key_on_policy(
	...
	ret =  tcs_create_seal_key_on_policy(key_path,type,policy);
...
int tcf_seal_data(
	...
	ret = tcs_seal_data(key_path,ibuffer,ilength,obuffer,olen_inout,passwd);
...
int tcf_unseal_data(
	...
	ret = tcs_unseal_data(key_path,ibuffer,ilength,obuffer,olen_inout,passwd);
...
int tcf_seal_data_store(
	...
	ret = tcs_seal_data_store(key_path,ibuffer,ilength,file_name,passwd);
...
int tcf_unseal_stored_data(
	...
	ret = tcs_unseal_stored_data(key_path,obuffer,olen_inout,file_name,passwd);
...
int tcf_get_sealed_data(
	...
	ret = tcs_get_sealed_data(key_path,obuffer,olen_inout,file_name);
...
int tcf_save_sealed_data(
	...
	ret = tcs_save_sealed_data(key_path,ibuffer,ilength,file_name);
...
int tcf_set_default_path_key_type(uint32_t type){
	...
	ret = tcs_set_default_path_key_type(type);
...
int tcf_create_migratable_path_key(unsigned char *key_path, uint32_t type, uint32_t source){
	...
	ret = tcs_create_migratable_path_key(key_path,type);
...
int tcf_create_path_key(unsigned char *key_path, uint32_t type, uint32_t source){
	...
	ret = tcs_create_path_key(key_path,type);
...
int tcf_get_public_key(
	...
	ret = tcs_get_public_key(key_path, pbuffer, obuf_len);
...
int tcf_change_leaf_auth(
	...
	ret = tcs_change_leaf_auth(key_path,oldpasswd,passwd);
...
int tcf_get_keyinfo(unsigned char *key_path, struct key_info *info){
	...
	ret = tcs_get_keyinfo(key_path,info);
...
int tcf_get_keyinfo_path(
	...
	ret = tcs_get_keyinfo_path(key_path,info,onumber_inout);
...
int tcf_create_shared_keytree_storespace(
	...
	ret = tcs_create_shared_keytree_storespace(ownerpass,size,nvpasswd);
	if(ret) return ret;
...
int tcf_remove_shared_keytree_storespace(unsigned char *ownerpass, uint32_t source){
	...
	ret = tcs_remove_shared_keytree_storespace(ownerpass);
	if(ret) return ret;
	httc_write_source_notices (source, POLICY_TYPE_STORE);
...
int tcf_save_shared_keytree(unsigned char *nvpasswd){
	...
	ret = tcs_save_shared_keytree(nvpasswd);
...
int tcf_load_shared_keytree(unsigned char *nvpasswd){
	...
	ret = tcs_load_shared_keytree(nvpasswd);
...
int tcf_set_private_keytree_storespace_index(uint32_t nvindex){
	...
	ret = tcs_set_private_keytree_storespace_index(nvindex);
...
int tcf_save_private_keytree(unsigned char *nvpasswd){
	...
	ret = tcs_save_private_keytree(nvpasswd);
...
int tcf_load_private_keytree(unsigned char *nvpasswd){
	...
	ret = tcs_load_private_keytree(nvpasswd);
...
int tcf_export_keytree(
	...
	ret = tcs_export_keytree(keypath,pbuffer,obuf_len);
...
int tcf_import_keytree(const char *keypath, unsigned char *pbuffer, int buf_len, uint32_t source){
	...
	ret = tcs_import_keytree(keypath,pbuffer,buf_len);
...
int tcf_read_keytree(const char *keypath, struct key_node **node,
	...
	ret = tcs_read_keytree(keypath,node,level);
...
int tcf_free_keynode(struct key_node **node, int recursive){
	...
	ret = tcs_free_keynode(node,recursive);
...
int tcf_delete_keytree(const char *keypath, uint32_t source){
	...
	ret = tcs_delete_keytree(keypath);
...
int tcf_get_migrate_auth(unsigned char **auth,	int *authlength){
	...
	ret = tcs_get_migrate_auth(auth,authlength);
...
int tcf_emigrate_keytree(const char *keypath, unsigned char *passwd, unsigned char *ownerpass, unsigned char *auth,	int authlength,unsigned char **pbuffer, int *obuf_len){
	...
	ret = tcs_emigrate_keytree(keypath,passwd,ownerpass,auth,authlength,pbuffer,obuf_len);
...
int tcf_immigrate_keytree(const char *keypath, unsigned char *pbuffer,int buf_len, uint32_t source){
	...
	ret = tcs_immigrate_keytree(keypath,pbuffer,buf_len);
