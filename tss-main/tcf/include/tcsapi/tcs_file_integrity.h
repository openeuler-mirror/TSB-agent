 ...
 * 	读取基准库
 */
int tcs_get_file_integrity(struct file_integrity_item **references, uint32_t *num, int *length);//proc 导出
 ...
 *	获取基准库有效条数
 */
int tcs_get_file_integrity_valid_number (uint32_t *num);//proc 导出

/*
 * 	获取基准库存储条数
 */
int tcs_get_file_integrity_total_number (uint32_t *num);//proc 导出

/*
 *	获取基准库可增量修改限制
 */
int tcs_get_file_integrity_modify_number_limit (uint32_t *num);//proc 导出
