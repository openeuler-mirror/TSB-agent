...
void usage()
{
	printf ("\n"
			...
			"		 -o <operaction>		- 0:tcf_get_keyinfo(default)  1:tcf_get_keyinfo_path\n"
...
int main(int argc, char **argv){
	...
	if(opt == 0){
		ret = tcf_get_keyinfo((unsigned char *)key_path, &info);
		if(ret){
	    ...
	    ret = tcf_get_keyinfo_path((unsigned char *)key_path,&infos,&number);
		if(ret){
