...
int tcf_read_mem_data(uint32_t index, int *length_inout, unsigned char *data, char *usepasswd);
...
int main(int argc, char **argv){
	...
	ret = tcf_read_mem_data(index,&length,data,usepasswd);
	if(ret){
		printf("tcf_read_mem_data error 0x%04X(%d)\n",ret,ret);
		return -1;
	}
