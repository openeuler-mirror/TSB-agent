...
//				printf("b:%d\n",b);
				x = atoi(optarg);
				if(x != 0){
					(*items + b)->be_remote_ip =  htonl((uint32_t)inet_addr(optarg));
//					printf ("remote_ip: 0x%08X\n", (uint32_t)inet_addr(optarg));
				}else{
...
//				printf("c:%d\n",c);
				x = atoi(optarg);
				if(x != 0){
					(*items + c)->be_local_ip =  htonl((uint32_t)inet_addr(optarg));
//					printf ("local_ip: 0x%08X\n", (uint32_t)inet_addr(optarg));
...
//				httc_util_dump_hex ("pubkey", pubkey , TPCM_PUBKEY_STR_LEN/2);
	...
	policy->be_server_ip = htonl((uint32_t)inet_addr(server_ip));
...
//	httc_util_dump_hex ("admin_auth_policy", update , length);
	...
	ret = tcf_update_tnc_policy(update,uid,cert_type,signlen,sign);
	if(ret){
		printf("[Error] tcf_update_tnc_policy ret:0x%08X\n",ret);
		ret = -1;
		goto out;
	}
	goto out;		
