...
void usage()
{
	printf ("\n"
			...
			"    -o <operaction>        - 0:tcf_create_path_key(default)  1:tcf_create_migratable_path_key\n"
...
int main(int argc, char **argv){
	...
	if(opt == 0){
		...
		ret = tcf_create_path_key((unsigned char *)keypath, type, source);
		if(ret){
		...
		ret = tcf_create_migratable_path_key((unsigned char *)keypath, type, source);
		if(ret){
			printf("Error tcf_create_migratable_path_key fail! ret:0x%08X(%d)!\n",ret,ret);
			return -1;
		}
	}
