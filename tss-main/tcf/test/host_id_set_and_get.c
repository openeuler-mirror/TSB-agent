...
int main(void)
{
	...
	printf("\n  1 host id no exist set \n");
	ret = httc_util_rm (HTTC_HOST_ID_FILE);
	if(ret != 0){
			printf("httc_util_rm %s\n", HTTC_HOST_ID_FILE);
			return TCF_ERR_FILE;
	}
	ret = tcf_set_host_id(set_id, set_len);
	if(ret) {
		printf("[tcs_set_host_id] ret: 0x%08x\n", ret);
		return -1;
	}
	httc_util_dump_hex ("set host id :",  set_id, set_len);	
	ret = tcf_get_host_id(get_id, &id_len);
	if(ret) {
	...
	/*2 host id exist set*/
	printf("\n  2 host id exist set \n");
	ret = tcf_set_host_id(set_2id, set_2len);
	if(ret) {
		printf("[tcs_set_host_id] ret: 0x%08x\n", ret);
		return -1;
	}
	httc_util_dump_hex ("set host id :",  set_2id, set_2len);	
	ret = tcf_get_host_id(get_2id, &id_2len);
	if(ret) {
		printf("[tcs_get_host_id] ret: 0x%08x\n", ret);
		return -1;
	}
