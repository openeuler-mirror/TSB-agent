...
//				printf ("passwd: %s\n", passwd);
	...
	ret = tcf_set_encrypt_key((unsigned char *)keypath, (unsigned char *)passwd,SM4_KEY_SIZE,(unsigned char *)key,source);
	if(ret){
		printf("Error tcf_set_encrypt_key fail ret:0x%016X!\n",ret);
		return -1;
	}
