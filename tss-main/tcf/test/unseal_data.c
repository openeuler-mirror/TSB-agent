...
//				printf ("filename: %s\n", filename);
	...
	ret = tcf_unseal_data((unsigned char *)key_path,(unsigned char *)ibuffer, (int)ilength,(unsigned char *)unsealbuffer,&unseallength,(unsigned char *)passwd);
	if(ret){
		printf("tcf_unseal_data fail! ret:0x%08X!\n",ret);
