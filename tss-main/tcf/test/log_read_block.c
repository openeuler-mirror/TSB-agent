...
{
	printf ("\n"
			" Usage: ./log_read_block -r <> -d<> -t<> \n"
			"    eg. ./log_read_block -r 10\n"
			"        ./log_read_block -r 10 -d 2 -t 20\n");
}
...
int read_logs_block (int read_num, int delete_num, int timeout)
{
	int ret = 0;
	int index = 0;
	struct log **logs = NULL;

	if (0 != (ret = tcf_read_logs (&logs, &read_num, timeout))){
...
int main (int argc , char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "r:d:t:h")) != -1)
	{
		switch (ch)
		{
			case 'r':
	...
	read_logs_block(read_logs_num, delete_logs_num, logs_timeout);
