...
//				printf ("filename: %s\n", filename);
	...
	ret = tcf_seal_data((unsigned char *)key_path,(unsigned char *)ibuffer, ilength,(unsigned char *)obuffer,&olen_inout,(unsigned char *)passwd);
	if(ret){
		printf("tcf_seal_data fail! ret:0x%08X!\n",ret);
		return -1;
	}
