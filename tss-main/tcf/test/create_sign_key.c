...
void usage()
{
	printf ("\n"
			...
			"		 -o <operaction>		- 0:tcf_create_sign_key(default) 1:tcf_create_sign_key_on_policy\n"
...
//				printf ("opt: %d\n", opt);
	...
	if(opt == 0){
		...
		ret = tcf_create_sign_key((unsigned char *)keypath, type, (unsigned char *)passwd,source);
		if(ret){
			printf("Error tcf_create_sign_key fail! ret:0x%08X (%d)!\n",ret,ret);
			return -1;
		}
	}else if(opt == 1){
		if(passwd == NULL ||
		...
		ret = tcf_create_sign_key_on_policy((unsigned char *)keypath, type, &policy,source);
		if(ret){
			printf("Error tcf_create_sign_key_on_policy fail! ret:0x%08X (%d)!\n",ret,ret);
			return -1;
		}
	}
