...
{	
	...
	item->path_length = with_path ? (item->is_full_path ? (strlen (path) + 1) : DEFAULT_HASH_SIZE) : 0;
...
int main (int argc, char **argv)
{
	...
	if (0 != (ret = tcf_prepare_update_critical_file_integrity (
			item, number, tpcm_id, tpcm_id_length, action, replay_counter, &references, &refLength))){
		printf ("tcf_prepare_update_critical_file_integrity error: %d(0x%x)\n", ret, ret);
		goto out;
	}
	...
	if (0 != (ret = tcf_update_critical_file_integrity (references, uid, auth_type, sigLen, sig))){
		printf ("tcf_update_critical_file_integrity error: %d(0x%x)\n", ret, ret);
		goto out;
	}
