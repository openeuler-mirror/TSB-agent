...
uint32_t policy_option = 0;
uint32_t policy_value = 0;

uint32_t pol[19] = {1, 0, 1, 0, 1, 1, 1, 1,PROCESS_MEASURE_MODE_TCS_MEASURE ,1,300,
...
{
	printf ("\n"
			" Usage: ./global_control_policy -a <auth_type> -k <key> -u <uid> -p <policy> -v <value>\n"
			...
			"    eg. ./tcf_global_control_policy -a 1 -k 60FFA139866D596A8E107C76F3B92BB32422ED302701FA6865B86CC82DAAFC2C09AA298AE0D2FF1A541C4CB02AF80D0C118F427A2623C5DB2D08A151D873CA99FD76A2F4BD9FA0D6FDB52BDE2124DB29B2B170E570A6E1BEE41138608F04750A -u 123123\n"
			"    eg. ./tcf_global_control_policy -a 1 -k 60FFA139866D596A8E107C76F3B92BB32422ED302701FA6865B86CC82DAAFC2C09AA298AE0D2FF1A541C4CB02AF80D0C118F427A2623C5DB2D08A151D873CA99FD76A2F4BD9FA0D6FDB52BDE2124DB29B2B170E570A6E1BEE41138608F04750A -u 123123 -p 0 -v 1\n\n");
}
...
//	uint8_t *PolicyStr = NULL;
	...
	while ((ch = getopt(argc, argv, "a:k:u:p:v:h")) != -1)
	{
		switch (ch) 
		{
			case 'a':
...
//				httc_util_dump_hex ("pubkey", pubkey , TPCM_PUBKEY_STR_LEN/2);
	...
	if(default_tag) ret = tcf_get_global_control_policy(&(policy->policy));
...
//	httc_util_dump_hex("after policy", &policy->policy, sizeof(struct global_control_policy));
	...
	ret = tcf_get_tpcm_id(policy->tpcm_id, &len);
	if(ret) goto out;
	...
	ret = tcf_set_global_control_policy(policy,(const char *)uid,auth_type,signlen,sign);
	if(ret){
		printf("[Error] tcf_set_global_control_policy ret:0x%08X\n",ret);
		ret = -1;
		goto out;
	}
