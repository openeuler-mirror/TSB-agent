...
{
	printf ("\n"
			" Usage: ./scan_file_integrity [options]\n"
			" options:\n"
			...
			"        -h 				- Help info\n\n"
			"    eg. ./scan_file_integrity -d /usr/bin/ -u file-integrity-uid -k 7638BA4A2C4E2C54D89A89D90C77E3EA71D83686B45B29265A26F99483F90AA9AEE3CAA6F0C75EEE048B6B6185BBA8C86FCA692A1750DAC49659E5FD5634407CD04979A36887BBF9B05CA0108D08329595AE0441BD5FA8E886AC07423E30B8FB\n"
			"    eg. ./scan_file_integrity -l 10 -d /usr/bin/ -u file-integrity-uid -k 7638BA4A2C4E2C54D89A89D90C77E3EA71D83686B45B29265A26F99483F90AA9AEE3CAA6F0C75EEE048B6B6185BBA8C86FCA692A1750DAC49659E5FD5634407CD04979A36887BBF9B05CA0108D08329595AE0441BD5FA8E886AC07423E30B8FB\n"
			"\n");
}
...
static int gui_num_limit = 60000;
	...
	while(1)
	{
		...
		if (wlDirent->d_type == 8) 
		{
			...
			if ((*number) && ((*number)%1000==0)){
				printf ("generate file_integrity number: %d\n", *number);
			}
		}
...
int main (int argc, char **argv)
{
	...
	int number = 0;
	int once_number = 1000;
	int already_number = 0;
	...
	httc_util_pr_dev ("httc_calloc item okay!\n");
  	while ((ch = getopt(argc, argv, "k:f:F:o:d:l:u:peh")) != -1)
	{
		switch (ch) 
		{
			case 'k':
			...
			case 'd':
				path = optarg;
				dir_generate_file_integrity_item (path, &total_number, flag, item);
	...
	do{
		number = (total_number - already_number) < once_number ? (total_number - already_number) : once_number;
		...
		if (0 != (ret = tcf_prepare_update_file_integrity (
				item + already_number, number, tpcm_id, tpcm_id_length,
				already_number ? POLICY_ACTION_ADD : POLICY_ACTION_SET,
				replay_counter, &references, &refLength))){
			printf ("tcf_prepare_update_file_integrity error: %d(0x%x)\n", ret, ret);
			goto out;
		}
		httc_util_pr_dev ("tcf_prepare_update_file_integrity item okay!\n");
		...
		if (0 != (ret = tcf_update_file_integrity (references, uid, auth_type, sigLen, sig,local_data,local_length))){
			printf ("tcf_update_file_integrity error: %d(0x%x)\n", ret, ret);
			goto out;
		}
		httc_free (references);
		
		already_number += number;
		printf ("### update_file_integrity %d items already ###\n", already_number);
	}while (already_number < total_number);
	
out:
	if (sig) SM2_FREE (sig);
	if (item){
		while (total_number--) httc_free (item[total_number].hash);
		httc_free (item);
	}
	return ret;
}
