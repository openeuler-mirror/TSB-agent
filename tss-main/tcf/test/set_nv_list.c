...
int main(int argc, char **argv){
	...
	infos = httc_util_file_read_full((unsigned char *)file, &length);
	if(infos == NULL) {
		printf("read error");
		return -1;
	}	
...
//	show_nv_list((struct nv_info *)array, number);
	...
	ret = tcf_set_nv_list((struct nv_info *)array,number,source);
	if(ret){
		printf("Error tcf_set_nv_list fail! ret:0x%08X(%d)!\n",ret,ret);
		if(infos) httc_free(infos);
		tcf_free_nv_list((struct nv_info *)array, number);
		return -1;
	}
	if(infos) httc_free(infos);
	tcf_free_nv_list((struct nv_info *)array, number);	
