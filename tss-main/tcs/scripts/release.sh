...
mkdir -p $cur_dir/PKG/kernel/include/
mkdir -p $cur_dir/PKG/user/
mkdir -p $cur_dir/PKG/user/include/
...
run_cmd make $EXTRA_FLAGS -j $CORE_NUM
...
cp tcsk_sm.h tcsk_tcm.h $cur_dir/PKG/kernel/include/
...
run_cmd make mock -j $CORE_NUM
run_cmd make -j $CORE_NUM
cp libhttc*.so $cur_dir/PKG/user/
cp $TCF_INCLUDE/tcsapi $cur_dir/PKG/user/include -frd
...
echo "fi" >> $cur_dir/PKG/install.sh
echo "if [ ! -d /usr/local/httcsec/include/ ]; then" >> $cur_dir/PKG/install.sh
echo "	mkdir -p /usr/local/httcsec/include/" >> $cur_dir/PKG/install.sh
echo "fi" >> $cur_dir/PKG/install.sh
#echo "cp ./user/include/* /usr/local/httcsec/include/ -frd" >> $cur_dir/PKG/install.sh
...
echo "cp ./user/tcm-lib/inc/* /usr/local/httcsec/include/ -frd" >> $cur_dir/PKG/install.sh
echo "rm /usr/local/httcsec/lib/ -fr" > $cur_dir/PKG/uninstall.sh
echo "rm  /usr/local/httcsec/include/	 -fr" >> $cur_dir/PKG/uninstall.sh
