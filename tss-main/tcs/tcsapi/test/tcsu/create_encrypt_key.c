...
void usage()
{
	printf ("\n"
			...
			"								  1:tcs_create_inner_encrypt_key \n"
			"								  2:tcs_create_encrypt_key_on_policy\n"
			"								  3:tcs_create_inner_encrypt_key\n"
...
//				printf ("opt: %d\n", opt);
	...
	if(opt == 0){
		...
		ret = tcs_create_encrypt_key((unsigned char *)keypath, type, (unsigned char *)passwd);
		if(ret){
		...
		ret = tcs_create_inner_encrypt_key((unsigned char *)keypath, type, (unsigned char *)passwd);
		if(ret){
		...
		ret = tcs_create_encrypt_key_on_policy((unsigned char *)keypath, type, &policy);
		if(ret){
			...
			policy.user_or_group = user_or_group;
			policy.password = passwd;		
			ret = tcs_create_inner_encrypt_key_on_policy((unsigned char *)keypath, type, &policy);
			if(ret){
				printf("tcm_create_seal_key fail! ret:0x%08X(%d)!\n",ret,ret);
				return -1;
			}
