...
//#include "../../../include/tcfapi/tcf_dev_version.h"
...
{
	printf ("\n"
			" Usage: ./update_dev_protect_policy [options]\n"
			" options:\n"
			"        -T <type>          - CDROM_FILE_PROTECT = 1, CDROM_FILE_NO_PROTECT = 0\n"
			...
			"    eg. ./update_dev_protect_policy   -T 0  -u dev-protect-uid -k 396D6C567AC5B374C93980893C532F4123195B9D19B927B0A69B5D81745C1C3E8D19F78081B017AF0E6E52198FDCD334F651265CEE28F300ABC3FE6D668BD1987E79916EACEE2BCF74C34ADE8F5A137C5E4F61CE30A362B8DDCD85FCF13A573A\n"
			"    eg. ./update_dev_protect_policy   -T 1  -u dev-protect-uid -k 396D6C567AC5B374C93980893C532F4123195B9D19B927B0A69B5D81745C1C3E8D19F78081B017AF0E6E52198FDCD334F651265CEE28F300ABC3FE6D668BD1987E79916EACEE2BCF74C34ADE8F5A137C5E4F61CE30A362B8DDCD85FCF13A573A\n"
			"\n");
}
...
int main(int argc,char **argv){
	...
	while ((ch = getopt(argc, argv, "a:T:c:u:k:h")) != -1)
	{
		switch (ch) 
		{
			case 'c':
...
//				httc_util_dump_hex ("pubkey", pubkey , TPCM_PUBKEY_STR_LEN/2);
	...
	length = HTTC_ALIGN_SIZE(sizeof(struct cdrom_protect_item),4);
	update = (struct cdrom_protect_update *)data;	
	
	update->be_size = htonl(sizeof(struct cdrom_protect_update));
	...
	policy = (struct cdrom_protect_item *)update->data;
	policy->be_flags=type;
	length += sizeof(struct cdrom_protect_update);
	...
	ret = tcs_update_cdrom_protect_policy(update,uid,cert_type,signlen,sign);
	if(ret){
