...
//				printf ("passwd: %s\n", passwd);
	...
	ret = tcs_get_encrypt_key((unsigned char *)keypath, (unsigned char *)passwd,&keylength,(unsigned char *)key);
	if(ret){
		printf("Error tcs_get_encrypt_key fail ret:0x%016X!\n",ret);
		return -1;
	}
