...
//				printf ("filename: %s\n", filename);
	...
	ret = tcs_unseal_stored_data((unsigned char *)key_path,(unsigned char *)unsealbuffer, &unseallength,(unsigned char *)filename,(unsigned char *)passwd);
	if(ret){
		printf("tcs_unseal_stored_data fail! ret:0x%08X!\n",ret);
		return -1;
	}
