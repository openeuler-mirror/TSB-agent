...
{
	printf ("\n"
			" Usage: ./nv_define_named_space [options]\n"
			" options:\n"
			...
			"    eg. ./nv_define_named_space -s 4000 -N one -w 123 -p 123456 -o 0\n"
			"    eg. ./nv_define_named_space -s 4000 -N two -w 123 -p 123456 -f 0x28 -n process -o 1\n"
			"\n");
}
...
//				printf ("opt: %d\n", opt);
	...
	if(opt == 0){
		...
		ret = tcs_nv_define_named_space((const char *)name, size,ownerpasswd,passwd);
		if(ret){
		...
		ret = tcs_nv_define_named_space_on_policy((const char *)name,size,ownerpasswd,&policy);
		if(ret){
