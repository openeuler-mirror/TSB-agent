...
{
	printf ("\n"
			" Usage: ./seal_data [options]\n"
			" options:\n"
			"	 	 -k	<keypath>			- The seal key path\n"
			"		 -d <data>			- Data to be encapsulated or unsealed\n"
			"		 -p <passwd>			- The key path password\n"
			"		 -s <filename>			- The name for sealfile\n"
			"		 -S <filename>			- The name for file to seal\n"
			"    eg. ./seal_data -k s://seal/a -d 123456789 -p 123456 -s Seal\n"
			"    eg. ./seal_data -k s://seal/b -d 123456789 -p 123456 -s Seal\n"
			"\n");
}
...
//				printf ("filename: %s\n", filename);
	...
	printf_time("tcs_seal_data start");
	ret = tcs_seal_data((unsigned char *)key_path,(unsigned char *)ibuffer, ilength,(unsigned char *)obuffer,&olen_inout,(unsigned char *)passwd);
	printf_time("tcs_seal_data end");
	if(ret){
		printf("tcs_seal_data fail! ret:0x%08X!\n",ret);
		return -1;
	}
