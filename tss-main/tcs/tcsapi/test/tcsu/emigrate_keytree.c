...
void usage()
{
	printf ("\n"
			...
			"		 -w <ownerpasswd>			- The passwd for owner\n"
...
int main(int argc, char** argv){
	...
	while((ch = getopt(argc, argv, "k:w:p:d:e:")) != -1){
		switch(ch){
			case 'k':
				key_path = optarg;
//				printf ("keypath: %s\n", key_path);
...
//				printf ("passwd: %s\n", passwd);
	...
	authlen = authlength;
	ret = tcs_emigrate_keytree((const char *)key_path,passwd,ownerpass, authdata,authlen,&obuffer,&olen_inout);
	if(ret){
		printf("tcs_emigrate_keytree fail!ret:0x%08X(%d)!\n",ret,ret);
