...
{
	printf ("\n"
			" Usage: ./update_critical_file_integrity  [options]\n"
			" options:\n"
			...
			"        -h 				- Help info\n\n"
			"    eg. ./update_critical_file_integrity -o 0 -F 0x05 -f /usr/bin/gdb -k 60FFA139866D596A8E107C76F3B92BB32422ED302701FA6865B86CC82DAAFC2C09AA298AE0D2FF1A541C4CB02AF80D0C118F427A2623C5DB2D08A151D873CA99FD76A2F4BD9FA0D6FDB52BDE2124DB29B2B170E570A6E1BEE41138608F04750A\n"
			"    eg. ./update_critical_file_integrity -o 1 -f /usr/bin/gdb -k 60FFA139866D596A8E107C76F3B92BB32422ED302701FA6865B86CC82DAAFC2C09AA298AE0D2FF1A541C4CB02AF80D0C118F427A2623C5DB2D08A151D873CA99FD76A2F4BD9FA0D6FDB52BDE2124DB29B2B170E570A6E1BEE41138608F04750A\n"
			"    eg. ./update_critical_file_integrity -o 1 -d /usr/bin -k 60FFA139866D596A8E107C76F3B92BB32422ED302701FA6865B86CC82DAAFC2C09AA298AE0D2FF1A541C4CB02AF80D0C118F427A2623C5DB2D08A151D873CA99FD76A2F4BD9FA0D6FDB52BDE2124DB29B2B170E570A6E1BEE41138608F04750A\n"
			"\n");
}
...
int main (int argc, char **argv)
{
	...
	char output[DEFAULT_HASH_SIZE] = {0};
	int olen = 0;
	...
	//sm3 (references->data, references->be_data_length, wlHash);
	//httc_util_dump_hex ("references->data", references->data, refLength);
	httc_util_dump_hex ("wlHash reference", wlHash, DEFAULT_HASH_SIZE);
#endif
...
//	httc_util_dump_hex ("references", references->data, refLength);
	...
	if ((ret = tcs_update_critical_file_integrity (references, uid, auth_type, sigLen, sig))){
		printf ("[tcs_update_critical_file_integrity ] ret: 0x%08x\n", ret);
		ret = -1;
	}
	gettimeofday (&end, NULL);
	used_usec = ((end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
	used_sec = used_usec / (float)1000000;
	printf ("  tcs_update_critical_file_integrity(%5s:0x%02x)  >>>  |number: %-10d|reflen(KB): %-10.2f|expend: %.02f\n",
