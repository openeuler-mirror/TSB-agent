...
//				printf ("passwd: %s\n", passwd);
	...
	ret = tcs_decrypt((unsigned char *)key_path,(unsigned char *)passwd, mode,(unsigned char *)decryptdata, decryptlength,(unsigned char *)obuffer,&olen_inout);
	if(ret){
		printf("tcm_encrypt fail!ret:0x%08X (%d)!\n",ret,ret);
