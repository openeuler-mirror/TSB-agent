...
int test_get_license_status(void)
{
	int ret = 0;
	uint32_t status = 0;
	uint32_t left = 0;
	...
	ret = tcsk_get_license_status((int *)&status, (int *)&left);
	if(ret) {
...
int get_license_status_init(void)
{
	int ret = 0;
	if((ret = test_get_license_status()) != 0) {
		printk("test_get_license_status, ret = %d\n", ret);
		return -1;
	}
	return 0;
}

void get_license_status_exit(void)
{
	printk ("[%s:%d]\n", __func__, __LINE__);
}

module_init(get_license_status_init);
module_exit(get_license_status_exit);
