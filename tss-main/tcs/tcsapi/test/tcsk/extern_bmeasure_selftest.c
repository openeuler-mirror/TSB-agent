...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("HTTC");
MODULE_DESCRIPTION("tcs_extern_boot_measure test");

extern int tcs_extern_boot_meausre (uint32_t pcr, uint32_t stage, uint32_t num,
...
int extern_bmeasure_test(uint32_t pcr, char *obj,int addr_num,int stage,int value)
{
	int i = 0;
	int ret = 0;
	uint32_t tpcmRes = 0;
	...
	ret = tcs_extern_boot_meausre (pcr, stage, addr_num, block, tdd_get_phys_addr ((void*)objAddress), objLen, &tpcmRes);
	printk ("[tcs_extern_boot_meausre]ret: 0x%08x, tpcmRes: 0x%08x\n", ret, tpcmRes);
...
int extern_bmeasure_selftest_init(void){
	int r;
	if((r = extern_bmeasure_test(0, "BIOSAPP 1/3", 1, 10, 0x10)))goto out;
	if((r = extern_bmeasure_test(1, "BIOSAPP 2/3", 2, 11, 0x11)))goto out;
	if((r = extern_bmeasure_test(1, "BIOSAPP 2/3", 3, 12, 0x12)))goto out;
	if((r = extern_bmeasure_test(2, "GRUB 1/3", 1, 1000, 0x1000)))goto out;
	if((r = extern_bmeasure_test(0, "GRUB 2/3", 2, 1001, 0x1001)))goto out;
	if((r = extern_bmeasure_test(2, "GRUB 1/3", 3, 1002, 0x1002)))goto out;
	if((r = extern_bmeasure_test(3, "Kernel 1/3", 1, 2000, 0x2000)))goto out;
	if((r = extern_bmeasure_test(3, "Kernel 2/3", 2, 2001, 0x2001)))goto out;
	if((r = extern_bmeasure_test(0, "Kernel 3/3", 3, 2002, 0x2002)))goto out;
out:
...
void extern_bmeasure_selftest_exit(void)
{
	printk ("[%s:%d]\n", __func__, __LINE__);
}

module_init(extern_bmeasure_selftest_init);
module_exit(extern_bmeasure_selftest_exit);
