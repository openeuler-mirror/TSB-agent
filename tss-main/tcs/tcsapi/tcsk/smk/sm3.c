...
int httc_sm3_init (sm3_context *ctx)
{
    if ( ctx == NULL ) {
        return -1;
    }
...
EXPORT_SYMBOL(httc_sm3_init);

int httc_sm3_update (sm3_context *ctx, const unsigned char *chunk_data, unsigned int chunk_length)
{
    uint32_t left, fill;
    uint32_t i;
    ...
    if (ctx->total_bytes_Low < chunk_length) {
...
EXPORT_SYMBOL(httc_sm3_update);

int httc_sm3_finish (sm3_context *ctx, unsigned char output[SM3_DIGEST_SIZE])
{
    ...
    ret = httc_sm3_update( ctx, SM3_padding, padn );
    if (ret != 0)
        return ret;

    ret = httc_sm3_update( ctx, msglen, 8 );
    if (ret != 0)
        return ret;
...
EXPORT_SYMBOL(httc_sm3_finish);

void httc_sm3 (const unsigned char *input, int ilen,
          unsigned char output[SM3_DIGEST_SIZE])
{
    sm3_context ctx;

    httc_sm3_init (&ctx);
    httc_sm3_update (&ctx, input, ilen);
    httc_sm3_finish (&ctx, output);

    memset (&ctx, 0, sizeof(sm3_context ));
}
EXPORT_SYMBOL(httc_sm3);
