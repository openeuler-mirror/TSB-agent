...
int ttm_sm3_init(ttm_sm3_context *index)
{
	if (index == NULL) {
		return -1;
	}
...
static void SM3_CF(ttm_sm3_context *index, const unsigned char *byte_64_block )
{
...
int ttm_sm3_finish(ttm_sm3_context *index, unsigned char output[SM3_DIGEST_SIZE])
{
	...
	ret = ttm_sm3_update(index, SM3_padding, padn);
	if (ret != 0)
		return ret;

	ret = ttm_sm3_update(index, msglen, 8);
	if (ret != 0)
		return ret;
...
int ttm_sm3_file(const char *path, unsigned char *output)
{
	...
	while ((n=fread(buf, 1, sizeof(buf), f)) > 0)
		ttm_sm3_update(&ctx, buf, (int)n);

	ttm_sm3_finish(&ctx, output);
